...
{
int numtasks, rank, buf, tag1=1, i, rc=0, dest, src, offset, nreqs;
double T1, T2;
...
if (rank < 2) {
  ...
  if (rank == 1) {
    src = 0;
    offset = 0;//offset should not extend array size
    }
  dest = src;

/* Do the non-blocking send and receive operations */
/* Note that the reqs array of each task has its own copy */ 
/* For task 0/1, reqs stores 20 requests respectively from offset 0-20*/
  for (i=0; i<REPS; i++) {
...
if (rank !=2) //task 2 is blocking call, no need for MPI_Wait
